...
export function createServer(): McpServer {
  ...
  server.registerTool(
    ...
    {
      description:
        "Post a message to a thread (record progress, reply, or feedback). Optional structured fields: `status` (started | in_progress | completed) plus optional objective/actions/reason for formatted output. You MUST post at least one message with your final result before the run ends. For blocked work items, set the thread work-item to blocked via `pixel_set_thread_status` if appropriate.",
      inputSchema: z.object({
        threadId: z.string().describe("Thread UUID"),
