...
- **Command:** `agent`
- **Workspace:** Pass `cwd` = the **agent’s own dir** (e.g. `agents/1-ceo/`). The CLI uses `--workspace <cwd>`, so that agent’s **MCP and skills** are loaded from that workspace (`.cursor/mcp.json` or `.claude/mcp.json`, plus `.agents/skills/`).
