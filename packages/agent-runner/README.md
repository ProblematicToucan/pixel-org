...
- **Command:** `agent`
- **Workspace:** Pass `cwd` = the **agent’s own dir** (e.g. `agents/1-ceo/`). The CLI uses `--workspace <cwd>`, so that agent’s **MCP and skills** (e.g. `mcp.json`, `skills/`) are loaded from that directory.
