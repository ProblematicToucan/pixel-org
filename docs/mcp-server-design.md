...
- Agent CLI runs with `--workspace` = agent dir (MCP + skills there).
...
| **Cursor/Claude integration** | No first-class integration; agent “decides” to HTTP. | MCP is first-class: one entry in `mcp.json`; agent uses tools like any other MCP. |
...
2. **Deployment flexibility**  
   - MCP server is typically run as a **stdio** process **spawned by the IDE/CLI** (see agent’s `mcp.json`).  
...
- **stdio**  
  - Cursor/Claude CLI spawn the MCP server from `mcp.json` (e.g. `node path/to/pixel-mcp-server.js`).  
  - Env can be set in `mcp.json` or by the orchestrator before starting the agent.  
...
- **HTTP/SSE**  
  - Backend (or a separate service) would expose an MCP endpoint; `mcp.json` would point to a URL.  
...
4. **Agent provisioning**
   - When creating an agent (DB + `ensureAgentDir`), write or update `mcp.json` in the agent dir with the Pixel MCP server entry and `PIXEL_AGENT_ID` (and optionally `PIXEL_BACKEND_URL` if not global).
